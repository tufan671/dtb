...
mongoose.connect('mongodb+srv://Arcenio:mea3jq14v1@cluster0.qyyew.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect bağlantısı.
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Pusha"} was here!`).setTimestamp();
  
  if (command === "evalll" && message.author.id === ayarlar.botOwner) {
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Arcenio Koruma" }).catch();
};
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    Database.find({guildID: ayarlar.guildID}).sort().exec((err, roles) => {
      roles.filter(r => !guild.roles.cache.has(r.roleID) && Date.now()-r.time > 1000*5*5*24*3).forEach(r => {//1 saatte bir alır. Süreyi değiştirebilirsiinz.
